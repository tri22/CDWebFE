...
const Home = () => {
    ...
    const fetchDashboardInfo = async () => {
        try {
            ...
            const [product, order, sale, allOrders] = await Promise.all([
                orderApi.bestSellingProduct(today),
                orderApi.weekTotalOrder(today),
                orderApi.weekTotalRevenue(today),
                orderApi.getAllOrder(), // Thêm API để lấy toàn bộ danh sách đơn hàng
            ]);

            // Đếm số đơn hàng có trạng thái CANCELLED
            ...
            setDashboardData({
                ...
                weekCancel: cancelledOrders, // Cập nhật số đơn hàng bị hủy
            });
        } catch (err) {
